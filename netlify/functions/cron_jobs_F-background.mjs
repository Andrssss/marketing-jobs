...
const JOB_NAME = "cron_jobs_F-background";
...
export default async (request) => {
  ...
  try {
      ...
      for (const it of items) {
        try {
          ...
          if (sourceKey === "frissdiplomas") {
            ...
            if (!keep) console.log(`[SKIP detail_check] ${it.title} | nem Budapest+marketing`);
          }
          if (!keep) continue;
          ...
          if (/HTTP\s+[45]\d{2}/i.test(err?.message)) {
            logFetchError(JOB_NAME, { url: it.url, message: err.message });
          }
          console.error(err);
        }
      }
    ...
    while (pagesProcessed < maxPages) {
      const pageUrl =
        page === 1
          ? "https://www.frissdiplomas.hu/allasok"
          : `https://www.frissdiplomas.hu/kereses/page:${page}`;
      try {
        ...
        logFetchError(JOB_NAME, { url: pageUrl, message: err.message });
